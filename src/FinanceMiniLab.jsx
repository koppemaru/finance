...
// 学生向け 財務三表ミニ・シミュレータ（1期間・簡易リンク）
// 前提と割り切り：
// - 単一期間のスナップショット。運転資本や減価償却以外の非現金費用の変動は無視。
// - 間接法CF：CFO = 当期純利益 + 減価償却（運転資本の変動は0と仮定）。
// - 期首現金は入力、投資CF(CFI)と財務CF(CFF)はスライダーで仮定。期末現金でB/Sを合わせる。
// - 細かい論点（繰延税金、少数株主持分、包括利益など）は割愛。

export default function FinanceMiniLab() {
  // ---- 入力（損益関連） ----
  const [sales, setSales] = useState(1000_000); // 売上高（円）
  const [cogsRate, setCogsRate] = useState(0.45); // 売上原価率
  const [sga, setSga] = useState(300_000); // 販管費
  const [dep, setDep] = useState(50_000); // 減価償却費
  const [interest, setInterest] = useState(10_000); // 支払利息
  const [taxRate, setTaxRate] = useState(0.30); // 実効税率

  // ---- 入力（期首・B/Sの素） ----
  const [begCash, setBegCash] = useState(200_000); // 期首現金
  const [ppeNet, setPpeNet] = useState(500_000); // 有形固定資産（純額）
  const [otherAssets, setOtherAssets] = useState(150_000); // その他資産
  const [debt, setDebt] = useState(250_000); // 有利子負債
  const [otherLiab, setOtherLiab] = useState(200_000); // その他負債
  const [shareCapital, setShareCapital] = useState(250_000); // 資本金等
  const [retainedEarningsBeg, setRetainedEarningsBeg] = useState(150_000); // 期首利益剰余金

  // ---- 入力（CF仮定）----
  const [cfi, setCfi] = useState(-60_000); // 投資CF（マイナス投資）
  const [cff, setCff] = useState(20_000); // 財務CF（調達>正、返済<負）
  const [dividend, setDividend] = useState(0); // 配当（利益剰余金から控除）
  ...
  const calc = useMemo(() => {
    ...
    const netChangeCash = cfo + cfi + cff; // 期中現金増減
  ...
  const Num = ({ value, onChange, step = 1000, min = -1_000_000, max = 5_000_000 }) => (
    <input
      type="number"
      className="w-full rounded-lg border px-3 py-2 bg-slate-50 focus:outline-none focus:ring-2"
      value={value}
      min={min}
      max={max}
      step={step}
